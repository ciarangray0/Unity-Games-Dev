            ...
            if(collider.gameObject.CompareTag("bullet")) { //if hit with a bullet
                ...
                if(this.bigAsteroid) { //if an original "big" asteroid is hit
                    for(int i = 0; i < 3; i++) {
                        ...
                        newAsteroid.GetComponent<Asteroid>().bigAsteroid = false; //no longer will be treated as a big asteroid
                    }
                        for(int i = 0; i < 4; i++) { //spawn 4 pieces of debris for visual effect
